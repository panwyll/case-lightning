-- ============================================================================
-- CONVEYi conveyancing engine — ONE-SHOT #3 for the Supabase SQL editor
-- Migration 071 (addendum 3): schema-level human gate + automation role, shadow
-- mode, per-sub-flow trust levels, shadow reviews. Run AFTER engine-one-shot-2.sql.
-- Idempotent; single transaction; records itself in schema_migrations.
--
-- After running, put your application role into the automation role so the engine can
-- step down for cron/webhooks/effects (the migration does this for conveyi_app and
-- app_rls if they exist; for any other role run:  grant conveyi_automation to <role>;).
-- ============================================================================
begin;
-- ────────────────────────────────────────────────────────────────────────────
-- 071_enforcement_and_shadow.sql
-- ────────────────────────────────────────────────────────────────────────────
...
insert into schema_migrations (filename) values ('071_enforcement_and_shadow.sql') on conflict (filename) do nothing;
commit;

-- Sanity check: expect 1 | t | t | t | 2
select
  (select count(*) from schema_migrations where filename = '071_enforcement_and_shadow.sql') as recorded,
  (select exists (select 1 from pg_trigger where tgname = 'matter_event_human_gate_trg')) as human_gate,
  (select exists (select 1 from pg_roles where rolname = 'conveyi_automation')) as automation_role,
  (select exists (select 1 from pg_policies where policyname = 'automation_no_human_gated_events')) as automation_policy,
  (select count(*) from pg_tables where tablename in ('engine_subflow_status','engine_shadow_review')) as shadow_tables;
