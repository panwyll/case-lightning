-- ============================================================================
-- CONVEYi conveyancing engine — ONE-SHOT #4 for the Supabase SQL editor
-- Migration 072: LEAP (leap.build) as the backend — connection, mirror columns,
-- write-back ledger. Run AFTER engine-one-shot-3.sql. Idempotent; single transaction.
-- ============================================================================
begin;
-- ────────────────────────────────────────────────────────────────────────────
-- 072_leap_backend.sql
-- ────────────────────────────────────────────────────────────────────────────
...
insert into schema_migrations (filename) values ('072_leap_backend.sql') on conflict (filename) do nothing;
commit;

-- Sanity check: expect 1 | 2 | 3
select
  (select count(*) from schema_migrations where filename = '072_leap_backend.sql') as recorded,
  (select count(*) from pg_tables where tablename in ('leap_connection','leap_writeback')) as leap_tables,
  (select count(*) from information_schema.columns where column_name in ('leap_matter_id','leap_document_id','leap_card_id')) as mirror_columns;
