...
test('ethical wall + handler-conflict rules hold at the database layer', { skip: !url ? 'DATABASE_URL not set' : false }, async () => {
  ...
  await runAsSystem(async () => {
    await svc.run(t, MB, { type: 'enrol', actor: B, hasLender: false, requiredSearches: ['CON29'], counterpartyType: 'internal' });
  ...
  await svc.run(t, MA, { type: 'enrol', actor: A, hasLender: false, requiredSearches: ['CON29'], counterpartyType: 'internal' });
