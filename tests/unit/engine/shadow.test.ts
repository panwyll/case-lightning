...
test('shadow matter: engine concludes, nothing surfaces, nothing is sent — every intent is logged as action_suppressed', async () => {
  const h = harness();
  await h.svc.run(TENANT, MATTER, { type: 'enrol', actor: USER, requireProofOfFunds: false, requireExchangeAuthority: false, hasLender: false, requiredSearches: ['CON29', 'LLC1'], shadowMode: true });
...
test('switching shadow mode off is a logged, people-only event; the matter then behaves normally', async () => {
  const h = harness();
  await h.svc.run(TENANT, MATTER, { type: 'enrol', actor: USER, requireProofOfFunds: false, requireExchangeAuthority: false, hasLender: false, requiredSearches: ['CON29'], shadowMode: true });
...
test('queue: one row per assigned matter, pending badge counts only surfaced blocking decisions, sortable by oldest pending or target completion', async () => {
  ...
  await h.svc.run(TENANT, MATTER, { type: 'enrol', actor: USER, requireProofOfFunds: false, requireExchangeAuthority: false, hasLender: false, requiredSearches: ['CON29'], targetCompletionDate: '2026-12-18' });
  await h.svc.run(TENANT, OTHER, { type: 'enrol', actor: USER, requireProofOfFunds: false, requireExchangeAuthority: false, hasLender: false, requiredSearches: ['CON29'], targetCompletionDate: '2026-11-06' });
