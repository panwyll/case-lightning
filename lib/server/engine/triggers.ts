...
export const TRIGGERS: TriggerSpec[] = [
  ...
  { id: 'both.pof_form_submitted', backend: 'both', source: 'user', feeds: 'command', label: 'Client submits the proof-of-funds form', description: 'Public, tokenised form (no login): the client declares each source, attaches evidence and signs the declarations. The submission becomes typed facts, rule flags, a declaration document and a sign-off decision for the conveyancer.', entry: 'app/api/v1/pof/[token] (POST)', implemented: true, reaches: ['proof_of_funds_submitted'] },
