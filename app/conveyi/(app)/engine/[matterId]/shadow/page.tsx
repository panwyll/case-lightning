...
import { KIND_LABEL, STAGE_LABEL, SUBFLOW_LABEL, fmtWhen, pretty, type DecisionRow } from '@/app/shared/engine/types';
    ...
    <div className="eg" style={{ maxWidth: 1180, margin: '0 auto', padding: '16px 16px 40px' }}>
      ...
      <div className="eg-top">
        <div>
          ...
          <p className="eg-sub">{c?.matter?.matterRef}{c?.matter?.handler ? ` · ${c.matter.handler}` : ''} · <a href={`/conveyi/engine/${matterId}`}>timeline</a> · <a href="/conveyi/engine/shadow">rollout board</a></p>
        </div>
              ...
              <div className="eg-card" style={{ marginBottom: 12 }}>
                ...
                {c.engine.decisions.map((d) => (
                  <div className="cmp-item" key={d.eventId}>
                    ...
                    <div className="m">{fmtWhen(d.createdAt)} · {d.status} · <a href={`/conveyi/decisions/${d.eventId}`}>open</a></div>
