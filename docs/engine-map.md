...
> Generated by `npm run engine:map` from `lib/server/engine/spec.ts` (`machineSpec()`), the same source the live map at `/engine/map` draws and `tests/unit/engine/spec.test.ts` checks against the machine. **Do not edit by hand** — change the machine and re-run. Spec version `2c2aada0115b`.

6 transaction types · 8 stages · 9 sub-flows · 81 commands · 108 event types · 13 decision kinds · 20 triggers · 60 eventualities · 45 issue kinds · 19 workstreams.
