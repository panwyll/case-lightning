...
> Generated by `npm run engine:map` from `lib/server/engine/spec.ts` (`machineSpec()`), the same source the live map at `/engine/map` draws and `tests/unit/engine/spec.test.ts` checks against the machine. **Do not edit by hand** — change the machine and re-run. Spec version `87f35552dfd4`.

6 transaction types · 8 stages · 9 sub-flows · 82 commands · 114 event types · 14 decision kinds · 26 triggers · 60 eventualities · 45 issue kinds · 19 workstreams.

## 0 · Transaction types

One machine, parameterised by a profile (`lib/server/engine/transactions.ts`, docs/transaction-types.md). The profile decides which phases a type passes through, which workstreams and sub-flows apply, who pays us, and what follows completion.

| Type | Side | Tenure | Exchange | Phases | Workstreams | Sub-flows | Money from | After completion | Note |
|---|---|---|---|---|---|---|---|---|---|
| **Freehold purchase** | buyer | freehold | yes | instruction → Searches & enquiries → Title & report → Pre-exchange → exchanged → pre_completion → completed → post_completion | id_aml, source_of_funds, title, searches, enquiries, mortgage, survey, contract, deposit, chain, report_on_title, co_ownership, completion, registration | id_check, search, enquiry, mortgage, title, report_on_title, chase, proof_of_funds | lender, client | ap1 | The buyer-side spine the engine was built on. |
| **Leasehold purchase** | buyer | leasehold | yes | instruction → Searches, pack & enquiries → Lease, title & report → Pre-exchange → exchanged → pre_completion → completed → post_completion | id_aml, source_of_funds, title, searches, enquiries, mortgage, survey, leasehold, contract, deposit, chain, report_on_title, co_ownership, completion, registration | id_check, search, enquiry, mortgage, title, report_on_title, chase, proof_of_funds, management_pack | lender, client | ap1 | Purchase plus the management pack, lease review and the notice of assignment. |
| **Freehold sale** | seller | freehold | yes | instruction → Contract pack → Enquiries & replies → Pre-exchange → exchanged → pre_completion → Completed — redeem & account → Discharge & close | id_aml, property_forms, title, enquiries, redemption, contract, chain, completion, discharge | id_check, title, chase | buyer_solicitor | discharge_only | Seller side: property forms from the client, contract pack out, the buyer's enquiries answered, the seller's mortgage redeemed and discharged. |
| **Leasehold sale** | seller | leasehold | yes | instruction → Contract pack & management pack → Enquiries & replies → Pre-exchange → exchanged → pre_completion → Completed — redeem & account → Discharge & close | id_aml, property_forms, title, leasehold, enquiries, redemption, contract, chain, completion, discharge | id_check, title, chase, management_pack | buyer_solicitor | discharge_only | Sale plus obtaining the management pack from the freeholder / agent for the buyer. |
| **Remortgage** | owner | any | none | instruction → Investigation → Ready to complete → Completed → Registration | id_aml, title, searches, mortgage, redemption, completion, registration, discharge | id_check, search, mortgage, title, chase | lender | ap1 | No exchange: title and offer investigated, the mortgage deed executed, the certificate of title sent, the old lender redeemed from the new advance, the new charge registered and the old one discharged. |
| **Transfer of equity** | owner | any | none | instruction → Investigation & consent → Execution → Completed → Registration | id_aml, title, lender_consent, co_ownership, completion, registration | id_check, title, chase | incoming_owner | ap1 | Ownership changes without a sale: identity of every party, the lender's consent where the property is charged, the basis of co-ownership decided by the clients (a declaration of trust where tenants in common), the transfer deed executed, SDLT where there is consideration, the transfer registered. |

```mermaid
flowchart LR
    subgraph freehold_purchase["Freehold purchase"]
        direction LR
        freehold_purchase_instruction["instruction"]
        freehold_purchase_pre_contract["Searches & enquiries"]
        freehold_purchase_instruction --> freehold_purchase_pre_contract
        freehold_purchase_contract_review["Title & report"]
        freehold_purchase_pre_contract --> freehold_purchase_contract_review
        freehold_purchase_pre_exchange["Pre-exchange"]
        freehold_purchase_contract_review --> freehold_purchase_pre_exchange
        freehold_purchase_exchanged["exchanged"]
        freehold_purchase_pre_exchange --> freehold_purchase_exchanged
        freehold_purchase_pre_completion["pre_completion"]
        freehold_purchase_exchanged --> freehold_purchase_pre_completion
        freehold_purchase_completed["completed"]
        freehold_purchase_pre_completion --> freehold_purchase_completed
        freehold_purchase_post_completion["post_completion"]
        freehold_purchase_completed --> freehold_purchase_post_completion
    end
    subgraph leasehold_purchase["Leasehold purchase"]
        direction LR
        leasehold_purchase_instruction["instruction"]
        leasehold_purchase_pre_contract["Searches, pack & enquiries"]
        leasehold_purchase_instruction --> leasehold_purchase_pre_contract
        leasehold_purchase_contract_review["Lease, title & report"]
        leasehold_purchase_pre_contract --> leasehold_purchase_contract_review
        leasehold_purchase_pre_exchange["Pre-exchange"]
        leasehold_purchase_contract_review --> leasehold_purchase_pre_exchange
        leasehold_purchase_exchanged["exchanged"]
        leasehold_purchase_pre_exchange --> leasehold_purchase_exchanged
        leasehold_purchase_pre_completion["pre_completion"]
        leasehold_purchase_exchanged --> leasehold_purchase_pre_completion
        leasehold_purchase_completed["completed"]
        leasehold_purchase_pre_completion --> leasehold_purchase_completed
        leasehold_purchase_post_completion["post_completion"]
        leasehold_purchase_completed --> leasehold_purchase_post_completion
    end
    subgraph freehold_sale["Freehold sale"]
        direction LR
        freehold_sale_instruction["instruction"]
        freehold_sale_pre_contract["Contract pack"]
        freehold_sale_instruction --> freehold_sale_pre_contract
        freehold_sale_contract_review["Enquiries & replies"]
        freehold_sale_pre_contract --> freehold_sale_contract_review
        freehold_sale_pre_exchange["Pre-exchange"]
        freehold_sale_contract_review --> freehold_sale_pre_exchange
        freehold_sale_exchanged["exchanged"]
        freehold_sale_pre_exchange --> freehold_sale_exchanged
        freehold_sale_pre_completion["pre_completion"]
        freehold_sale_exchanged --> freehold_sale_pre_completion
        freehold_sale_completed["Completed — redeem & account"]
        freehold_sale_pre_completion --> freehold_sale_completed
        freehold_sale_post_completion["Discharge & close"]
        freehold_sale_completed --> freehold_sale_post_completion
    end
    subgraph leasehold_sale["Leasehold sale"]
        direction LR
        leasehold_sale_instruction["instruction"]
        leasehold_sale_pre_contract["Contract pack & management pack"]
        leasehold_sale_instruction --> leasehold_sale_pre_contract
        leasehold_sale_contract_review["Enquiries & replies"]
        leasehold_sale_pre_contract --> leasehold_sale_contract_review
        leasehold_sale_pre_exchange["Pre-exchange"]
        leasehold_sale_contract_review --> leasehold_sale_pre_exchange
        leasehold_sale_exchanged["exchanged"]
        leasehold_sale_pre_exchange --> leasehold_sale_exchanged
        leasehold_sale_pre_completion["pre_completion"]
        leasehold_sale_exchanged --> leasehold_sale_pre_completion
        leasehold_sale_completed["Completed — redeem & account"]
        leasehold_sale_pre_completion --> leasehold_sale_completed
        leasehold_sale_post_completion["Discharge & close"]
        leasehold_sale_completed --> leasehold_sale_post_completion
    end
    subgraph remortgage["Remortgage"]
        direction LR
        remortgage_instruction["instruction"]
        remortgage_pre_contract["Investigation"]
        remortgage_instruction --> remortgage_pre_contract
        remortgage_pre_completion["Ready to complete"]
        remortgage_pre_contract --> remortgage_pre_completion
        remortgage_completed["Completed"]
        remortgage_pre_completion --> remortgage_completed
        remortgage_post_completion["Registration"]
        remortgage_completed --> remortgage_post_completion
    end
    subgraph transfer_of_equity["Transfer of equity"]
        direction LR
        transfer_of_equity_instruction["instruction"]
        transfer_of_equity_pre_contract["Investigation & consent"]
        transfer_of_equity_instruction --> transfer_of_equity_pre_contract
        transfer_of_equity_pre_completion["Execution"]
        transfer_of_equity_pre_contract --> transfer_of_equity_pre_completion
        transfer_of_equity_completed["Completed"]
        transfer_of_equity_pre_completion --> transfer_of_equity_completed
        transfer_of_equity_post_completion["Registration"]
        transfer_of_equity_completed --> transfer_of_equity_post_completion
    end
```
...
| `record_acknowledgement` | automation | any | `acknowledgement_sent` | Something arrived from the other side or the client and they were told so (recorded only after the send). |
...
| `record_note` | person | any | `note_recorded` | File a note, a dictated note or a call transcript on the matter. It is evidence from the moment it lands, whatever the reading makes of it. |
| `note_action_refused` | automation | any | `note_action_refused` | A line a person approved that the machine then refused (a precondition was never there). Recorded so the note never claims something landed that did not. |
| `note_extracted` | automation | any | `note_extracted` | What the note appears to say, as proposals. Each must quote the note verbatim and map to a command the machine already accepts, or it is dropped. Raises one decision for a person; nothing is applied until they approve. |
| `request_property_forms` | either | any (freehold_sale, leasehold_sale) | `property_forms_requested` | Sale: ask the client for the TA6 / TA10 (and TA7 on a leasehold); a wait the timers chase. |
| `property_forms_received` | either | any (freehold_sale, leasehold_sale) | `property_forms_received` | Sale: the completed forms are in (facts extracted where a document arrived). |
| `contract_pack_sent` | either | pre_contract, contract_review, pre_exchange, exchanged, pre_completion, completed, post_completion (freehold_sale, leasehold_sale) | `contract_pack_sent` | Sale: draft contract, official copies, plan and forms out to the buyer's solicitor. Needs the forms and the title on file. |
| `buyer_enquiries_received` | either | any (freehold_sale, leasehold_sale) | `buyer_enquiries_received` | Sale: the buyer's enquiries on the pack, numbered BE1…; each is a reply we owe (gates contract_review). |
| `enquiry_replies_sent` | person | any (freehold_sale, leasehold_sale) | `enquiry_replies_sent` | Sale: replies went out under a person's name. |
| `request_redemption_statement` | either | any (freehold_sale, leasehold_sale, remortgage) | `redemption_statement_requested` | Sale / remortgage with an existing charge: ask the lender for the redemption figure; a wait the timers chase. |
| `redemption_statement_received` | either | any (freehold_sale, leasehold_sale, remortgage) | `redemption_statement_received` | The redemption figure, its validity date and daily interest are on file. |
| `mortgage_redeemed` ⛔ | either | any (freehold_sale, leasehold_sale, remortgage) | `mortgage_redeemed` | On or after completion, and only after a person authorised the payment to the lender against verified details (hard stop). |
| `discharge_confirmed` | either | any (freehold_sale, leasehold_sale, remortgage) | `discharge_confirmed` | The lender's DS1 / e-DS1 is through and the charge is off the register. |
| `mortgage_deed_executed` | either | any (freehold_purchase, leasehold_purchase, remortgage) | `mortgage_deed_executed` | Lender-funded purchase / remortgage: the client signed the mortgage deed, witnessed. An unwitnessed deed is refused (raise a document-execution issue). |
| `certificate_of_title_sent` | person | any (freehold_purchase, leasehold_purchase, remortgage) | `certificate_of_title_sent` | The solicitor's certificate to the lender requesting the advance; follows a resolved offer. |
| `request_lender_consent` | either | any (transfer_of_equity) | `lender_consent_requested` | Transfer of equity on a charged property: the lender's consent to the transfer (or a deed of substituted security); a wait the timers chase. |
| `lender_consent_received` | either | any (transfer_of_equity) | `lender_consent_received` | Consent and any conditions on file. |
| `transfer_deed_executed` | either | any (freehold_purchase, leasehold_purchase, transfer_of_equity) | `transfer_deed_executed` | The TR1 / TP1 signed by every party, witnessed. |
| `deed_of_trust_executed` | either | any (freehold_purchase, leasehold_purchase, transfer_of_equity) | `deed_of_trust_executed` | Co-owners holding as tenants in common: the declaration of trust signed. Needs the ownership_basis client decision first; refused for joint tenants. |
| `sdlt_not_required` | person | completed, post_completion (freehold_purchase, leasehold_purchase, remortgage, transfer_of_equity) | `sdlt_not_required` | A person determines no SDLT return is due (e.g. a transfer of equity for no chargeable consideration), with the reason; unblocks AP1. |

🔒 human-gated at the database (automation cannot write it). ⛔ hard stop while a bank-details decision is pending.

## 5 · Event types

- **lifecycle**: `matter_created`, `stage_advanced`, `manual_handling_required`, `deposit_received`, `exchange_conditions_met`, `contracts_exchanged`, `completion_statement_generated`, `completion_confirmed`, `sdlt_submitted`, `ap1_submitted`, `ap1_confirmed`, `acknowledgement_sent`, `action_proposed` (decision), `action_approved`, `action_rejected`, `action_failed`, `sdlt_not_required`
...
- **notes & calls**: `note_recorded`, `note_extracted` (decision), `note_actions_applied`, `note_action_refused`
- **decisions**: `decision_source_opened`, `auto_clear_proposed` (decision), `auto_clear_review_raised` (decision), `auto_clear_confirmed`
...
- **sale**: `property_forms_requested`, `property_forms_received`, `contract_pack_sent`, `buyer_enquiries_received`, `enquiry_replies_sent`
- **redemption**: `redemption_statement_requested`, `redemption_statement_received`, `mortgage_redeemed`, `discharge_confirmed`
- **deeds**: `mortgage_deed_executed`, `certificate_of_title_sent`, `transfer_deed_executed`, `deed_of_trust_executed`
- **transfer of equity**: `lender_consent_requested`, `lender_consent_received`
...
| `property_forms` | 5 wd | 3 wd | 12 wd | 5 wd | client |
| `redemption` | 3 wd | 3 wd | 8 wd | 3 wd | lender |
| `lender_consent` | 5 wd | 5 wd | 15 wd | 5 wd | lender |
| `discharge` | 10 wd | 10 wd | 30 wd | 10 wd | lender |
...
| `note_actions` | approve · reject · escalate | the note or call transcript it was read from |
| `proposal` | approve · reject | the chase / deadline dossier |
...
| Email filed to a case from the filing queue | native | user | ingest | search, enquiry, mortgage, title, id_check | yes |
| Case created / updated in InTouch | both | webhook | sync | — | yes |
| Identity check completed in InTouch | both | provider | command | request_id_check, id_check_result | yes |
| Client completes a property form (TA6/TA7/TA10/TA13) | both | user | command | property_forms_received | yes |
| Client uploads a document to InTouch | both | file | ingest | search, mortgage, title, id_check | yes |
| Milestone pushed to the client portal | both | user | comms | — | yes |
...
- **Propose before trusting** — Every action the engine takes on its own has a trust level per firm: propose (a person approves each one), assist (the mechanical goes unasked), auto. New firms start at propose for everything. _(service.ts suppressed(), store.ts surfacedDecisions)_
...
_lib/server/engine/spec.ts (checked against machine.ts, types.ts, rules.ts, sla.ts, triggers.ts, transactions.ts by tests/unit/engine/spec.test.ts)_