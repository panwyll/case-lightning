...
- [x] **Close the Data-API door** — `024_lockdown_data_api.sql` written: revokes all `public` access from `anon`/`authenticated` (current + future) and enables deny-all RLS on every table. *(Note: the waitlist route uses the **service_role** key over the Data API, which bypasses RLS — so we lock the public roles, NOT disable the Data API, which would break signups.)* **Run on prod**, then sanity-check waitlist signup + the app still work.
